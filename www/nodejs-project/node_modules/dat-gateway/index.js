...
class DatGateway extends DatLibrarian {
  ...
  load () {
    ...
    return this.getHandler().then((handler) => {
      ...
      this.websocketServer.on('error', () => console.log('errored'));
    }).then(() => {
  ...
  getWebsocketHandler () {
    return (stream, req) => {
      ...
      // req.on('error', (e) => {
      //     console.error('error in dat-gateway getWebsocketHandler: ' + e.message);
      // });
      // stream.on('error', console.log("getWebsocketHandler error"));
      // stream.on('error', console.error);
      // stream.on('error', function (e) {
      //     console.log('getWebsocketHandler handling error: ' , e);
      // });
      const urlParts = req.url.split('/')
      log("urlParts: " + urlParts)
      ...
      if (address === 'peers') {
        ...
        return this.add(address).then((dat) => {
          ...
          console.log("Error in getWebsocketHandler: " + e)
    ...
    return this.getIndexHtml().then((welcome) => {
      return (req, res) => {
        ...
        let hostnameParts = urlParts.hostname.split('.')
          console.log("pathParts: " + pathParts + " hostnameParts: " + hostnameParts)
        ...
        console.log("getHandler address: " + address + " isRedirecting: " + isRedirecting + " subdomain.length: " + subdomain.length)
        ...
        req.on('error', (e) => {
            console.error('error in dat-gateway getHandler req: ' + e.message);
        });

        res.on('error', (e) => {
            console.error('error in dat-gateway getHandler res: ' + e.message);
        });
  ...
  resolveName (address) {
    console.log("resolveName: " + address)
